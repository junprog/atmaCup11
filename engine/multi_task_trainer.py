...
from models.efficient_net import MultiBranchEfficientNet, MultiBranchEfficientNet_cls, MultiBranchEfficientNet_v2, MultiBranchEfficientNet_v3
from models.nf_net import MultiBranchNFNet
from models.vision_transformer import MultiBranchViT
from models.fusion_net import DoubleNet, TripleNet
from losses.multi_task_loss import MultiTaskLoss, MultiTaskLoss_cls, MultiTaskLoss_v2, MultiTaskLoss_v3
...
class MultiTaskTrainer(Trainer):
    def setup(self):
        ...
        #self.criterion = MultiTaskLoss_cls()
    ...
    def train(self):
        ...
        for i, (train, val) in enumerate(self.skf.split(self.train_df['object_id'], self.train_df['target'])):
            ...
            if 'resnet' in args.arch:
                ...
                self.model = MultiBranchEfficientNet_cls(arch=args.arch, num_classes=4, mate_classes=self.mate_class + 1, tech_classes=self.tech_class + 1)
                ...
                self.model = MultiBranchNFNet(arch=args.arch, num_classes=1, mate_classes=self.mate_class + 1, tech_classes=self.tech_class + 1)
            elif 'vit' in args.arch:
                self.model = MultiBranchViT()
                print()
            elif 'fusion' in args.arch:
                resnet_path = os.path.join('logs_multitask/exp09-resnet-0721-112249', 'cv_{}'.format(i), 'best_model.pth')
                efnet_path = os.path.join('logs_multitask/exp05-long(mse)-0715-003341', 'cv_{}'.format(i), 'best_model.pth')
                vit_path = os.path.join('/home/junya/Documents/MoCo_v3_pytorch/checkpoint_0300.pth.tar')
                #self.model = DoubleNet(resnet_weight_path=resnet_path, efnet_weight_path=efnet_path, freeze=True)
                self.model = TripleNet(resnet_weight_path=resnet_path, efnet_weight_path=efnet_path, vit_weight_path=vit_path, freeze=True)
            ...
            if args.init_weight_path:
                ...
                if suf == 'tar':
                    ...
                    if 'vit' in args.arch:
                        self.model.vit.load_state_dict(new_checkpoint)
                    else:
            ...
            # for params in self.model.vit.parameters():
    ...
    def train_epoch(self, epoch, i):
        ...
        #for inputs, target, mate, tech in tqdm(self.train_loader, ncols=60):
            ...
            with torch.set_grad_enabled(True):
                ...
                #loss = self.criterion(outputs, logit, mate, tech)
...
class MultiTaskTrainer_hard(Trainer):
    def setup(self):
        """initialize the datasets, model, loss and optimizer"""
        args = self.args
    ...
    def train(self):
        ...
        for i, (train, val) in enumerate(self.skf.split(self.train_df['object_id'], self.train_df['target'])):
            ...
            if args.cv_weight_dir:
                cv_checkpoint = torch.load(os.path.join(args.cv_weight_dir, 'cv_{}'.format(i), 'best_model.pth'))
                self.model.load_state_dict(cv_checkpoint)
            ...
            self.scheduler = lr_scheduler.MultiStepLR(self.optimizer, milestones=[100], gamma=0.1)
            #self.scheduler = lr_scheduler.CosineAnnealingLR(self.optimizer, T_max=args.max_epoch)
